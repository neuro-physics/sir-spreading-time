...
[A_FL,caseName_FL] = loadAALMatrix('D:\Dropbox\p\postdoc\data\AALmatrix_N306\selected_FL\TLE_*_FL_AAL_midres.mat');
...
fl_c = mean(fl_z_c,2,'omitnan');
fl_p = mean(fl_z_p,2,'omitnan');
tt_fl = ttest.ttest2Struct(fl_c,fl_p,'Alpha',p_thresh,'AlphaFDR',p_thresh);
[fh_fl,ax] = plotAvgVsThetaScatterPanels({{'FL values'}},fl_c,fl_p,[],[],tt_fl.h,[],[],'Z-Score',{'Controls','TLE'});

if saveOutputFigures
    saveFigure(fh_fl, fullfile('corr_fig','group_diff_fl'), 'tiff', true, [], 600);
end
...
[fhR,axh] = plot_func.plot_corrcoef_datastruct('\theta','FA-FL corr',[0,1.1],p_thresh,...
...
xlabel(ax_p,'FL values (Z-score)');
ylabel(ax_c,'FA values (Z-score)');
xlabel(ax_c,'FL values (Z-score)');
...
if saveOutputFigures
    saveFigure([fhR,fh_p,fh_c], fullfile('corr_fig',{'corr_group_diff_fl_fa','corr_fl_fa_z_tle','corr_fl_fa_z_control'}), 'tiff', true, [], 600);
end
...
if saveOutputFigures
    saveFigure(fh1, fullfile('corr_fig','group_diff_map_fl_in_pval'), 'tiff', true, [], 600);
end
...
[fhR,axh] = plot_func.plot_corrcoef_datastruct('\theta',['R (FL and ',weightedTxt,'in sp times)'],[0,1.1],p_thresh,...
...
[fhR,axh] = plot_func.plot_corrcoef_datastruct('\theta',['R (FL and ',weightedTxt,'out sp times)'],[-0.6,0.2],p_thresh,...
...
legend(axh);
title(axh,'FL Values (Z-score)','FontWeight','normal');
...
xlabel(axin_p,'FL values (Z-score)');
ylabel(axin_p,'In-Spreading Times (Z-score)');
xlabel(axin_c,'FL values (Z-score)');
...
xlabel(axout_p,'FL values (Z-score)');
ylabel(axout_p,'Out-Spreading Times (Z-score)');
xlabel(axout_c,'FL values (Z-score)');
...
if saveOutputFigures
    saveFigure([fhR,fhin_p,fhout_p,fhin_c,fhout_c], fullfile('corr_fig',{'corr_sp_fl_z','corr_sptime_in_fl_z_tle','corr_sptime_out_fl_z_tle','corr_sptime_in_fl_z_con','corr_sptime_out_fl_z_con'}), 'tiff', true, [], 600);
end
...
[r_c_specific_fl_sr,p_c_specific_fl_sr,...
    ...
    r_p_specific_fl_sr,p_p_specific_fl_sr,...
    r_p_fl_sr,p_p_fl_sr,theta,in_scplot] = ...
    corrcoef_datastruct(si.(spField),npm_fake_for_FL,'flipData','SAvg','FL',false,true,si.caseName,'FL values');

[fhR,axh] = plot_func.plot_corrcoef_datastruct('\theta',['R (FL and ',weightedTxt,'sp times)'],[0,1.1],p_thresh,...
    r_c_specific_fl_sr,p_c_specific_fl_sr,r_c_fl_sr,p_c_fl_sr,r_p_specific_fl_sr,p_p_specific_fl_sr,r_p_fl_sr,p_p_fl_sr,theta,...
...
if saveOutputFigures
    saveFigure([fhR,fhin_p,fhin_c], fullfile('corr_fig',{'corr_sptime_fl_z','corr_sptime_fl_z_tle','corr_sptime_fl_z_con'}), 'tiff', true, [], 600);
end