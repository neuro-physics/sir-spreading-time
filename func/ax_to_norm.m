...
function [xN,yN] = ax_to_norm(xAx,yAx,axh,fh)
    ...
    xN = x_to_norm_v2(xAx(1),xAx(2),axh,fh);
    yN = y_to_norm_v2(yAx(1),yAx(2),axh,fh);
end

function [y_norm] = y_to_norm_v2(y_begin,y_end,axh,fh)

%if nargin ~= 2
%     error('Wrong number of input arguments! y_to_norm_v2(y_begin,y_end)')
%end

h_axes = get(fh,'CurrentAxes');    %get axes handle.
axesoffsets = get(h_axes,'Position');%get axes position on the figure. 
y_axislimits = get(axh, 'ylim');     %get axes extremeties.
y_dir = get(axh,'ydir');
...
%********************************************************
function [x_norm] = x_to_norm_v2(x_begin,x_end,axh,fh)
...
x_axislimits = get(axh, 'xlim');     %get axes extremeties.
x_dir = get(axh,'xdir');
