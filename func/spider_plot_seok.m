function [f, ca, o] = spider_plot_seok(data,tle,rng,lbl,leg,f)
...
% make the plot
axColor = 0.7 .* ones(1,3);
% define the axis locations
start = [zeros(1,r); cos(ang')]; stop = [zeros(1,r); sin(ang')];
% plot the axes
plot(ca,start,stop,'color',axColor,'linestyle','-'); axis equal
% plot axes markers
inc = 0.25:.25:1; mk = .02 * ones(1,4); tx = 3 * mk; tl = 0:.25:1;
% loop each axis ang plot the line markers and labels
% add axes
for ii = 1:r
	% plot tick marks
    tmX = [(cos(ang(ii)) * inc + sin(ang(ii)) * mk); (cos(ang(ii)) * inc - sin(ang(ii)) * mk)];
    tmY = [(sin(ang(ii)) * inc - cos(ang(ii)) * mk); (sin(ang(ii)) * inc + cos(ang(ii)) * mk)];
	tm = plot(ca,tmX,tmY,'color',axColor);
	% label the tick marks
    if hasCommonScale && (ii == 1)
        for jj = 1:4
...
function c = myColorMap(N)
    T = hex2rgb({'#6a3d9a','#33a02c','#1f78b4','#e31a1c','#ff7f00','#b15928','#a6cee3','#b2df8a','#fb9a99','#fdbf6f','#cab2d6','#ffff99'});
    nBase = size(T,1);
    if N <= nBase
        c = T(1:N,:);
        return;
