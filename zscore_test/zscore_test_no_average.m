...
[t.h,~,~,t.st] = ttest2(cData,pData,'Alpha',0.05);
...
s_z = std(z,[],2,'omitnan');
cData_z = reshape(z(cInd,:),numel(cInd)*N2,1);
pData_z = reshape(z(pInd,:),numel(pInd)*N2,1);
[t_z.h,~,~,t_z.st] = ttest2(cData_z,pData_z,'Alpha',0.05);
...
cData_zz = reshape(zz(cInd,:),numel(cInd)*N2,1);
pData_zz = reshape(zz(pInd,:),numel(pInd)*N2,1);
[t_zz.h,~,~,t_zz.st] = ttest2(cData_zz,pData_zz,'Alpha',0.05);
...
cData_zc = reshape(zc(cInd,:),numel(cInd)*N2,1);
pData_zc = reshape(zc(pInd,:),numel(pInd)*N2,1);
[t_zc.h,~,~,t_zc.st] = ttest2(cData_zc,pData_zc,'Alpha',0.05);
