...
[fh2,ax] = plotBrain(pos,s,[],[],saveFrames,'',brainShadeAlpha,jet,viewAngle);
p = ax(end).Position;
cbh = colorbar(ax(end),'Location','southoutside','Position',[0.0369 0.8983 0.3459 0.0173],'Limits',minmax(s(end,:)),'TickDirection','out','AxisLocation','in');
set(cbh.Label,'String','$S_{ij}$','Interpreter','latex','FontSize',16);
ax(end).Position = p;
ax(end).Position(2) = 0.01;
