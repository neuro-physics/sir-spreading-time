function [ F, R, P ] = FindBestFunctionalMatch(As, Af, Theta, nSeedsRange, nSim, tTotal, comparisonMethod)
...
function [ F, R, P ] = FindBestFunctionalMatch_internal(As, Af, Theta, nSeedsRange, nSim, tTotal, bestMatchCondition, comparisonMethod)
    ...
    for i = 1:numOfTestSeeds
        Ftemp = FunctionalConnGuess(As, Theta, nSeedsRange(i), nSim, tTotal);
        [R(i),P(i)] = comparisonMethod(Ftemp(:), Af(:));
