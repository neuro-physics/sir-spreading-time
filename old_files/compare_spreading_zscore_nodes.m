...
%shortPathFile = 'D:\Dropbox\p\postdoc\data\sirs\old_2016-11-06\N90\shortPath.mat';
...
%     y = { zScoreAvgControls{j}, zScoreAvgPatients{j}, zScoreAvg{j} };
    ...
    psZScore(i) = getPlotStruct(x,y,yErr,...
        'Nodes, $n$', 'Average $z$-score', 'linear', 'linear', ...
...
psDiff = getPlotStruct(xDiff,yDiff,cell(size(xDiff)),...
    'Nodes, $n$', '$|z_c-z_p|$', 'linear', 'linear', ...
    '\theta', theta, ones(size(xDiff)), ['average spreading time z-score difference; \theta=',num2str(theta(i))]);
...
for i = 1:numel(indSigDiff)
    if ~isempty(indSigDiff{i})
        for j = 1:numel(indSigDiff{i})
            k = indSigDiff{i}(j);
            text(k, yDiff{i}(k), aal_labels{k}, 'Interpreter', 'none', 'Rotation', 90, 'Color', pLineColorsDiff(i,:));
        end
    end
end
