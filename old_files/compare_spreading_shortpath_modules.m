...
% nedaSelected = false;
% sortType = 'comm';
% nSigma = [1,1,1,0]; % number of deviations from the mean in order to be considered a stand out
...
%nComm = numel(unique(cm.C{gIndex}));
for i = 1:numel(sp)
...
%     end
    ...
    slowControlsM{j} = slowControlsM{j};
    ...
    slowPatientsM{j} = slowPatientsM{j};
end
...
for i = 1:nFiles
    ...
    DMeanControlsMaxInd_node{i} = find(DMeanControls_node{i}>(mean(DMeanControls_node{i})+nSigma(i).*std(DMeanControls_node{i})));
    DMeanPatientsMaxInd_node{i} = find(DMeanPatients_node{i}>(mean(DMeanPatients_node{i})+nSigma(i).*std(DMeanPatients_node{i})));
end
...
[fh(2),~,~] = plotPlotStruct([], psDMean_theta, {':'}, 1, pSymbols, lines(2), ...
    ...
    {'Box', 'on', 'XLim', [-0.005,0.055], 'YLim', [-5, 70], 'Layer', 'top'},... % axis properties
...
psDMean_node = getPlotStruct(x, y, cell(1,2*nFiles), ...
   'Community, $n$', 'Mean Hamming distance per comm, $\langle H(S_n,P_n)\rangle$', 'linear', 'linear', '\theta', legVal, zeros(1,2*nFiles), 'Mean Hamming distance per comm');
% psDMean_node = getPlotStruct(x, y, cell(1,2*nFiles), ...
%     'Community, $n$', 'Mean Hamming distance per comm, $\langle H(S_n,P_n)\rangle$', 'linear', 'linear', '\theta', legVal, ones(1,2*nFiles), 'Mean Hamming distance per comm');
    %'Community, $n$', 'Mean Hamming distance per comm, $\langle H(S_n,P_n)\rangle$', 'linear', 'log', '\theta', legVal, zeros(1,2*nFiles), 'Mean Hamming distance per comm');
...
if ~isempty(k)
    ...
    for k = m
        for i = 1:numel(DMeanControlsMaxInd_node{k})
            ...
            text(j, DMeanControls_node{k}(j), aal_labels{j}, 'Interpreter', 'none', 'Rotation', 90, 'Color', pLineColors(1,:));
        end
    end
end
...
if ~isempty(k)
    ...
    for k = m
        for i = 1:numel(DMeanPatientsMaxInd_node{k})
            ...
            text(j, DMeanPatients_node{k}(j), aal_labels{j}, 'Interpreter', 'none', 'Rotation', 90, 'Color', pLineColors(2,:));
        end
    end
end
...
psPNaNFull = getPlotStruct(nNaN_full, PNaN_full, cell(1,nFiles), ...
    'Community, $n$', '\% cases with failed seed nodes, $q(n)$', 'linear', 'linear', '\theta', theta, ones(1,nFiles), '% cases with failed seed nodes');
...
for k = 1:nFiles
    if any(PNaN_full{k})
        ...
        if numel(ind) > 5
            ind = find(PNaN_full{k} > ( mean(PNaN_full{k}) + nSigma(k).*std(PNaN_full{k})   )    );
        end
        for i = ind
            text(i, PNaN_full{k}(i), aal_labels{i}, 'Interpreter', 'none', 'Rotation', 90, 'Color', pLineColors(k,:)); 
        end
    end
end
...
psPNaNControls = getPlotStruct(nNaN_controls, PNaN_controls, cell(1,nFiles), ...
    'Community, $n$', '\% controls with failed seed comm, $q_{controls}(n)$', 'linear', 'linear', '\theta', theta, ones(1,nFiles), '% controls with failed seed comm');
...
for k = 1:nFiles
    if any(PNaN_controls{k})
        ...
        if numel(ind) > 5
            ind = find(PNaN_controls{k} > ( mean(PNaN_controls{k}) + nSigma(k).*std(PNaN_controls{k})  )   );
        end
        for i = ind
            text(i, PNaN_controls{k}(i), aal_labels{i}, 'Interpreter', 'none', 'Rotation', 90, 'Color', pLineColors(k,:)); 
        end
    end
end
...
psPNaNPatients = getPlotStruct(nNaN_patients, PNaN_patients, cell(1,nFiles), ...
    'Community, $n$', '\% patients with failed seed comm, $q_{patients}(n)$', 'linear', 'linear', '\theta', theta, ones(1,nFiles), '% patients with failed seed comm');
...
for k = 1:nFiles
    if any(PNaN_patients{k})
        ...
        if numel(ind) > 5
            ind = find(PNaN_patients{k} > (  mean(PNaN_patients{k}) + nSigma(k).*std(PNaN_patients{k})    )    );
        end
        for i = ind
            text(i, PNaN_patients{k}(i), aal_labels{i}, 'Interpreter', 'none', 'Rotation', 90, 'Color', pLineColors(k,:)); 
        end
    end
end
...
psPNaNCases_theta = getPlotStruct(x, y, cell(size(x)), ...
    '$\theta$', '\% cases with failed seed comm, $q(\theta;n)$', 'linear', 'linear', 'n', [1:N,1:N], zeros(size(x)), '% cases with failed seed comm');

k = nFiles;
if any(PNaN_controls{k})
    indC = find(PNaN_controls{k});
    if numel(indC) > 5
        indC = find(PNaN_controls{k} > (mean(PNaN_controls{k}) + nSigma(k).*std(PNaN_controls{k})   )   );
    end
end
if any(PNaN_patients{k})
    indP = find(PNaN_patients{k});
    if numel(indP) > 5
        indP = find(PNaN_patients{k} > (mean(PNaN_patients{k}) + nSigma(k).*std(PNaN_patients{k})   )    );
    end
end
...
[fh(7),~,~]=plotPlotStruct([], psPNaNCases_theta, pLineSt, 2, 'o', pLineColors, ...
    {'LineWidth', 1, 'MarkerSize', 3},... % plot properties
    {'ShowErrorBar', 'off'},... % error properties
    {'Box', 'on', 'XLim', [-0.005,0.055], 'YLim', [-0.02,1], 'Layer', 'top'},... % axis properties
    {'Location', 'best', 'FontSize', 9, 'Box', 'off', 'Interpreter', 'latex'},... % legend properties
    {'FontSize', 12}); % label properties
