...
nSigmaPlus = 1.5; % number of deviations from the mean in order to be considered a stand out
nSigmaMinus = 1.4;
ggamma = 2.0;
...
% nedaSelected = false;
% sortType = 'comm';
% nSigmaPlus = 1.2; % number of deviations from the mean in order to be considered a stand out
% nSigmaMinus = 1.2;
...
%%
for j = 1:nFiles
    zScores_plus{j} = selectZScores(sp(j).SAvg, nSigmaPlus, @(a,b)a>=b);
    zScores_minus{j} = selectZScores(sp(j).SAvg, -nSigmaMinus, @(a,b)a<=b);
...
for i = 1:nFiles
    ...
    create3PanelsImageFig({ A1, A2, A3 }, { 'Total', 'Controls', 'TLE' }, titleText);
