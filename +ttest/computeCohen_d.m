...
n1       = size(x1,1);
n2       = size(x2,1);
mean_x1  = nanmean(x1,1);
mean_x2  = nanmean(x2,1);
var_x1   = nanvar(x1,[],1);
var_x2   = nanvar(x2,[],1);
...
if isIndependent
    ...
    pooledSD(pooledSD==0) = 1;
    d        =  meanDiff ./ pooledSD; % Cohen's d (for independent samples)
...
elseif isPaired
    ...
    sdDeltas = nanstd(deltas,[],1);  % standard deviation of the diffferences
    s        = sdDeltas;        % re-name
